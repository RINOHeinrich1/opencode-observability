...
import { readFileSync, existsSync, statSync, createReadStream, openSync, closeSync, unlinkSync } from "node:fs";
...
// Agrège l'usage d'une session opencode (tokens + coût, par modèle et par agent)
// depuis `opencode export <sessionId>`. Résultat mis en cache 60 s.
...
async function taskConsumption(taskId) {
  const db = registry();
  let sessions = [];
  try {
